...
This work focuses on improving the underlying~\glspl{ipa}, especially the running time and proof size of the protocol are of interest.
The following is our approach to how we modified the~\glspl{ipa}, with focus on the~\gls{dlipa}.
...
In \autoref{fig:fold}(a) is a scheme function that simply pads the vector to the next power of two before running an~\gls{ipa}.
If one wanted to run current~\glspl{ipa} on vectors that are not a power of two, this would generally be the easiest way to achieve that.
...
It is worth mentioning that using the folding as shown in~\autoref{fig:fold}(b) results in the second recursive round being a size corresponding to a power of two.
...
First, the vectors are, on line 13, divided into the two sets,~$T$ and $S$, as in~\autoref{lst:ipa-prover}.
...
The second event that can occur is related to the fact that every shuffle distributes the probability of a certain ciphertext to be in a certain slot.
...
CAAUrdleproofs differs on this topic in regard to the~\glspl{ipa},~\gls{sameperm} and~\gls{samemsm}.
