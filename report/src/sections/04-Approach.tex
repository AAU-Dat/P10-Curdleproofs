...
Implementing the above explained protocol introduced some optimizations required to have the code run as fast as possible.
...
Include AccumulateCheck and AccumulateVerify functions?
...
$\textbf{Step 3:}$
$\mathbf{\gamma}\gets(\gamma_1,...,\gamma_m)$ hmmm
for $1\leq j\leq n$:
    $\textit{Compute s: see below for difference}$
