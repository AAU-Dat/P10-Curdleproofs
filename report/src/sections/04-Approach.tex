...
At first, step 1, the verifier sets up to run the protocol.
...
For instance, if some of the cups are the adversary's own.
...
\begin{figure}[!htb]
        \begin{lstlisting}[language=Python,mathescape=true,label={lst:ipa-verifier-optimized},numbers=right,caption={Optimized verifier computation for CAAU-IPA in CAAUrdleproofs},captionpos=b,frame=single]
$\textbf{Step 1:}$
